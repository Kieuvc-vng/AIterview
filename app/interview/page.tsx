...
function InterviewContent() {
  ...
  const config = configParam ? decodeConfig(configParam) : null;

  if (!config) {
    return <InvalidLink />;
  }

  return <InterviewChat config={config} />;
}
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="text-center space-y-4">
        ...
        <h1 className="text-2xl font-bold">{t("error.invalidLink", "vi")}</h1>
        <p className="text-[var(--color-text-muted)]">{t("error.invalidLinkDesc", "vi")}</p>
        <Link
          href="/"
...
function InterviewChat({ config }: { config: InterviewConfig }) {
  ...
  function finishInterview() {
    ...
    try {
      localStorage.setItem("interview-result", JSON.stringify(result));
    } catch {}
    setPhase("finished");
  }
  ...
  function handleExport() {
    try {
      const stored = localStorage.getItem("interview-result");
      if (!stored) return;
      const blob = new Blob([stored], { type: "application/json" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `interview-result-${config.jobTitle.replace(/\s+/g, "-")}.json`;
      a.click();
      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        ...
        >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-3 text-sm font-semibold text-white hover:shadow-lg transition-all"
          >
            <Download className="h-4 w-4" />
            {t("interview.export", lang)}
          </button>
        </motion.div>
      </div>
    );
